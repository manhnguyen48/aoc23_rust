...
fn look_around(lines: &[&[u8]], i: usize, j: usize) -> Option<Vec<(usize, usize)>> {
    ...
    for di in (i.saturating_sub(1))..=(i + 1).min(lines.len() - 1) {
        for dj in (j.saturating_sub(1))..=(j + 1).min(lines[di].len() - 1) {
            let mut start_j = dj;
            let line = &lines[di];
            while start_j > 0 && line[start_j - 1].is_ascii_digit() && line[dj].is_ascii_digit() {
                start_j -= 1;
            }
            if lines[di][start_j].is_ascii_digit() {
